...
if [[ ! -f /var/lib/postgresql/docker-configured ]]; then
	...
	if [ -f /tmp/import-pgsql/import-georchestra.sql ]; then
		...
		if [ "$(cat /tmp/geofence-exists)" == "(Aucune ligne)" ]; then
			...
			psql -d georchestra -c "UPDATE geofence.gf_gsinstance SET baseURL = 'https://$GEORCHESTRA_HOSTNAME/geoserver', password = '$GEOSERVER_USER_PASS', username = 'geoserver_privileged_user' WHERE name = 'default-gs';"
		fi
	else
